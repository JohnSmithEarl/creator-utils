...
(function () {
    ...
    for (let i = 0; i < 256; i++) {
        ...
        let t1 = (d[sx] * 0x101) ^ (sx * 0x1010100);
        SUB_MIX_0[x] = (t1 << 24) | (t1 >>> 8);
        SUB_MIX_1[x] = (t1 << 16) | (t1 >>> 16);
        SUB_MIX_2[x] = (t1 << 8) | (t1 >>> 24);
        SUB_MIX_3[x] = t1;

        // Compute inv sub bytes, inv mix columns tables
        let t2 = (x8 * 0x1010101) ^ (x4 * 0x10001) ^ (x2 * 0x101) ^ (x * 0x1010100);
 ...
 */
export class AlgoAES extends BlockCipher {
    static readonly keySize = 256 / 32;

    _nRounds: number = 0;
    _keyPriorReset: WordArray = undefined;
    _keySchedule: Array<number> = undefined;
    _invKeySchedule: Array<number> = undefined;
